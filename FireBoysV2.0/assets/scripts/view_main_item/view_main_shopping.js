cc.Class({
    ...
    refreshCoinCount(){
        ...
        this.node.getChildByName('view_coin').getChildByName('view_coinCount').getComponent(cc.Label).string = coinCount;
    },
    ...
    getItemInfo(){
        let self = this;
        boy.getLocalData.getItemData({
            getSuccess(data){
                self.jsonObj = data;
            }
        })
        for(let i in self.jsonObj){
            this.itemInfos.push([this.jsonObj[i],'itemList']);
        }
    ...
    addItem(itemNode,itemData,itemType,shoppingNode){
        itemNode.getComponent('view_main_shopping_item').initByData(itemData,itemType,shoppingNode);
    },

    exitBtn(){
        this.node.getChildByName('exit').on(cc.Node.EventType.TOUCH_END,(event)=>{
            this.node.active = false;
        });
    },

    nextPage(){
        this.node.getChildByName('view_main_shopping_right').on(cc.Node.EventType.TOUCH_END,(event)=>{
            this.state = 0;
    ...
    beforePage(){
        this.node.getChildByName('view_main_shopping_left').on(cc.Node.EventType.TOUCH_END,(event)=>{
            this.state = 1;
            if(this.index > 0){
                this.index -= 5;
            }
