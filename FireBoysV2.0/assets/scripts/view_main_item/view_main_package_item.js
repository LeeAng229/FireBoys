...
cc.Class({
    ...
    refreshItemHadNum(){
        this.node.getChildByName('item_number').getChildByName('item_number').getComponent(cc.Label).string = this.hadNum;
    },
    ...
    sellBtn(){
        ...
        let self = this;
        //获取当前道具的售出的价格
        let itemSell = Number(this.node.sellPrice);
        //获取当前拥有的金币数量
        let coinCount = 0;
        ...
        boy.getReqData.sellItem({
            ...
            reqSuccess(data){
                if(data){
                    ...
                    self.hadNum -= 1;
                    self.node.parent.parent.getComponent('view_main_package').refreshPage();
                    self.node.parent.parent.getComponent('view_main_package').refreshPageCount();
                }
            }
        })

        boy.getReqData.addCoinNum(this.node.sellPrice);
    },
